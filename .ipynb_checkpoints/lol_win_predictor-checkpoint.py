...
def load_and_preprocess_data(players_file='players_stats.csv', teams_file='teams_stats.csv'):
    """
    Load and preprocess the player and team data
...
def save_models(feature_to_win_rate_model, player_to_feature_models, key_features, filename='lol_models.pkl'):
    """
    ...
    for model_type, model_dict in feature_to_win_rate_model.items():
        if model_type == 'neural_network':
            ...
            model.save(f'win_rate_{model_type}_model')
            model_dict['model_path'] = f'win_rate_{model_type}_model'
    ...
    for feature, feature_models in player_to_feature_models.items():
        for model_type, model_dict in feature_models.items():
            if model_type == 'neural_network':
                model = model_dict['model']
                model_dict['model'] = None  # Temporarily set to None for serialization
                model.save(f'{feature}_{model_type}_model')
                model_dict['model_path'] = f'{feature}_{model_type}_model'
...
def load_models(filename='lol_models.pkl'):
    """
